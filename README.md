...
Blocking is **animated, not static** (#218): entering a block plays a readable ~0.14 s "raise weapon to guard" sweep with a slight overshoot-settle (per direction), and while you hold the block the weapon keeps a subtle direction-flavoured "living guard" motion — a lateral rock for Left/Right, a vertical brace for Overhead, a forward pulse for Stab — instead of freezing. The motion is deterministic (driven off the block's tick clock), so it stays in lockstep across the first-person viewmodel and the third-person model.

In third-person mode, every combatant's weapon is attached through one shared helper, `attachThirdPersonWeapon` (#220), which seats the weapon on the right hand's `weapon_attach` bone and applies a small per-weapon **grip** (a purpose-built third-person grip map, separate from the camera-space viewmodel grips). The grip is composed onto the bone rather than the weapon mesh, so the swept damage tracers and the visible blade move together — damage always lands where the blade visually passes. The same helper is used by the player, warmup bots, training dummies, and remote players, so held-weapon orientation is identical everywhere and can't drift between spawn paths.
...
- **Central stone plateau** — a 36×36 m flat top raised to `y ≈ 4` with a walkable smoothstep skirt (radius ≈ 26 m). It is the foundation for the **castle** (below).
- **Castle** (`src/arena/createCastle.ts`) — a low-poly medieval keep on the plateau: 30×30 m curtain walls with **walkable ramparts** (wall tops at `y ≈ 10`), a segmented **gatehouse** with a 4 m opening flanked by gate towers, four corner towers, a central keep, and two courtyard staircases whose step rise (≤ `AUTOSTEP_MAX_HEIGHT`) lets you climb to the ramparts with plain W. Client-only geometry (flat boxes + 1:1 Rapier cuboid colliders); the headless server needs none of it. Plus **map-wide medieval props** (`src/arena/props.ts`) — a market stall on the gate approach, carts, hay bales, fences, a ruined wall, and barrels.
