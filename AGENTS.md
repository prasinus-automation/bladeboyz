...
│   ├── world/
│   │   └── SpawnPoints.ts       # Spawn-point registry + selectSpawnPoint() weighted-random selector + seedPlaceholderSpawnPoints() (#93, #134)
│   ├── animation/
...
- **Out of scope**: HUD subscribers (`DeathScreen` / `Killfeed` / `Scoreboard` — issue C, `RespawnEvent` is now emitted on the EventBus and ready to consume); real arena geometry (#91 — placeholders here are the seam, no other code changes needed when it lands); real `dropEquippedWeapon` body (#94 / #A2 — still a no-op stub from #130).
