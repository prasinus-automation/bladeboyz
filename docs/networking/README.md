...
| 04  | `04-server-runtime-and-deploy.md` *(planned, see #138)*                    | Server runtime, deploy, handshake byte layout  | Node.js entry point, how the existing fixed-timestep loop is reused server-side, Docker image, port mapping, smoke tests, plus the byte-level handshake layout and the `DisconnectReason` / `PlayerLeftReason` enums (relocated from doc 03's earlier scope). |
